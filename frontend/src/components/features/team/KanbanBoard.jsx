...
const COLUMNS = [
    ...
    { title: 'In Progress', colId: 'IN_PROGRESS',  statusLabel: 'In Progress', badgeVariant: 'primary',   accent: '#6366f1', bg: 'from-indigo-50/70 to-violet-50/40',     border: 'border-indigo-200',  dot: 'bg-indigo-400',  countBg: 'bg-indigo-100 text-indigo-600' },
    { title: 'Done',        colId: 'DONE',         statusLabel: 'Done',        badgeVariant: 'success',   accent: '#10b981', bg: 'from-emerald-50/70 to-teal-50/40',      border: 'border-emerald-200', dot: 'bg-emerald-400', countBg: 'bg-emerald-100 text-emerald-600' },
    { title: 'Blocked',     colId: 'BLOCKED',      statusLabel: 'Blocked',     badgeVariant: 'danger',    accent: '#f43f5e', bg: 'from-rose-50/70 to-pink-50/40',          border: 'border-rose-200',    dot: 'bg-rose-400',    countBg: 'bg-rose-100 text-rose-600' },
];
...
const ImportJiraModal = ({ teamId, onClose, onImported }) => {
        ...
        api.get('/jira/status')
            .then(r => {
                if (r.data.connected && r.data.projects?.length > 0) {
                    ...
                    setSelectedKey(r.data.projects[0].key);
                } else {
                    setError('Jira not connected or no projects found. Please connect Jira first.');
                }
            })
            .catch(() => setError('Failed to fetch Jira projects.'))
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await api.post('/jira/import-issues', { teamId, projectKey: selectedKey });
            setResult(res.data);
            onImported(res.data.tasks || []);
        } catch (err) {
    ...
    return (
        <Modal onClose={onClose}>
            <ModalHeader title="Import from Jira" icon={<Download className="w-4 h-4" />} onClose={onClose} />
            <div className="px-6 py-5 space-y-4">
                {loading ? (
                    ...
                    <div className="text-center py-4">
                        ...
                        <p className="font-bold text-slate-800">{result.imported} tasks imported!</p>
                        <p className="text-sm text-slate-500 mt-1">{result.skipped} already existed — skipped.</p>
                    </div>
                ) : (
                    <FormField label="Select Jira Project">
                        ...
                        <p className="text-xs text-slate-400 mt-2">
                            All issues from this project will be imported as tasks. Already imported issues will be skipped.
                        </p>
                    </FormField>
                )}
            </div>
            <div className="flex gap-2.5 px-6 pb-6 justify-end">
                <button onClick={onClose} className="px-4 py-2.5 text-sm font-semibold text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">
                    {result ? 'Close' : 'Cancel'}
                </button>
                {!result && !error && !loading && (
                    <button onClick={handleImport} disabled={importing || !selectedKey}
                        className="px-5 py-2.5 text-sm font-semibold text-white bg-indigo-500 rounded-xl hover:bg-indigo-600 active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2">
                        {importing ? <><Loader2 className="w-4 h-4 animate-spin" /> Importing…</> : <><Download className="w-4 h-4" /> Import Issues</>}
                    </button>
                )}
            </div>
        </Modal>
...
const ExportJiraModal = ({ teamId, onClose }) => {
    const [projects, setProjects]       = useState([]);
    const [selectedKey, setSelectedKey] = useState('');
    const [loading, setLoading]         = useState(true);
        ...
        api.get('/jira/status')
            .then(r => {
                if (r.data.connected && r.data.projects?.length > 0) {
                    ...
                    setError('Jira not connected. Please connect Jira from Integrations page.');
                }
            })
            .catch(() => setError('Failed to fetch Jira projects.'))
            .finally(() => setLoading(false));
    ...
    const handleExport = async () => {
        ...
        try {
            const res = await api.post('/jira/export-tasks', { teamId, projectKey: selectedKey });
            setResult(res.data);
        } catch (err) {
    ...
    return (
        <Modal onClose={onClose}>
            <ModalHeader title="Export Tasks to Jira" icon={<Download className="w-4 h-4" />} onClose={onClose} />
            <div className="px-6 py-5 space-y-4">
                {loading ? (
                    <div className="flex items-center justify-center py-8">
                        <Loader2 className="w-6 h-6 animate-spin text-indigo-400" />
                    ...
                    <div className="text-center py-4">
                        ...
                        <p className="font-bold text-slate-800 text-lg">{result.exported} tasks exported! 🎉</p>
                        {result.failed > 0 && <p className="text-sm text-red-400 mt-1">{result.failed} tasks failed.</p>}
                        <p className="text-sm text-slate-500 mt-2">Check your Jira project — issues have been created.</p>
                    </div>
                ) : (
                    <>
                        <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-sm text-indigo-700">
                            All tasks from this board will be exported as issues to the selected Jira project. Already synced tasks will be skipped.
                        </div>
                        <FormField label="Select Jira Project">
                            <select className={inputCls} value={selectedKey} onChange={e => setSelectedKey(e.target.value)}>
                                {projects.map(p => (
            ...
            <div className="flex gap-2.5 px-6 pb-6 justify-end">
                ...
                {!result && !error && !loading && (
                    <button onClick={handleExport} disabled={exporting || !selectedKey}
                        className="px-5 py-2.5 text-sm font-semibold text-white bg-indigo-500 rounded-xl hover:bg-indigo-600 active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2">
                        {exporting ? <><Loader2 className="w-4 h-4 animate-spin" /> Exporting…</> : <><Download className="w-4 h-4" /> Export All Tasks</>}
                    </button>
                )}
            </div>
        </Modal>
...
const AddTaskModal = ({ columnStatus, teamId, onClose, onCreated }) => {
    const [form, setForm] = useState({ title: '', description: '', priority: 'Medium', status: columnStatus, assignee: '', confidence: '' });
...
const EditTaskModal = ({ task, onClose, onUpdated }) => {
    const [form, setForm] = useState({ title: task.title || '', description: task.description || '', priority: task.priority || 'Medium', assignee: task.assignee?._id || task.assignee || '' });
    const [loading, setLoading] = useState(false);
    const [error, setError]     = useState('');
    ...
    return (
        <Modal onClose={onClose}>
            ...
            <div className="px-6 py-5 space-y-4">
                <FormField label="Title"><input className={inputCls} value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} /></FormField>
                <FormField label="Description"><textarea className={`${inputCls} resize-none h-20`} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} /></FormField>
                <FormField label="Priority">
                    <select className={inputCls} value={form.priority} onChange={e => setForm(p => ({ ...p, priority: e.target.value }))}>
                        {['High', 'Medium', 'Low'].map(p => <option key={p}>{p}</option>)}
                    </select>
                            ...
                            return (
                                <div key={h._id || i} className="flex gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors items-start">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm text-slate-700 font-medium">Moved to <span className="font-semibold" style={{ color: col?.accent || '#64748b' }}>{labelFor(h.status)}</span></p>
                                        {h.changedAt && <p className="text-xs text-slate-400 mt-0.5">{new Date(h.changedAt).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}</p>}
                                    </div>
                                </div>
                            );
                        })}
...
const KanbanBoard = ({ searchQuery = '' }) => {
    ...
    const [importModal, setImportModal] = useState(false); // ← Jira import
    const [exportModal, setExportModal] = useState(false); // ← Jira export
    ...
    const handleImported = (newTasks) => {
        setTasks(prev => {
            ...
            const fresh = newTasks.filter(t => !existingIds.has(t._id));
            return [...fresh, ...prev];
        });
    };
    ...
    if (error) return (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <button onClick={fetchTasks} className="px-4 py-2 text-sm font-semibold text-white bg-indigo-500 rounded-xl hover:bg-indigo-600 transition-colors">Retry</button>
        </div>
    );
    ...
    return (
        <>
            <div className="flex gap-4 overflow-x-auto h-[calc(100vh-8rem)] pb-4 px-1"
                ...
                {COLUMNS.map(col => (
                    <KanbanColumn key={col.colId} col={col} tasks={tasksFor(col.colId)}
                        onDrop={handleDrop} onAddTask={(colId) => setAddModal(colId)}
                        onEdit={(task) => setEditModal(task)} onDelete={handleDelete}
                        onViewHistory={(id) => setHistModal(id)}
                        onImportJira={() => setImportModal(true)}
                        onExportJira={() => setExportModal(true)} />
                ))}
            </div>
